...
<template>
    <main flex="~ col" bg-white w-full max-w-96 p-8 rounded-xl shadow="md black/5">
        ...
        <main py-4>
            <div flex="~" items-center text-3xl px-4 py-2 bg-truegray-100 rounded-xl border="~ truegray-200"
                ...
                <div v-show="!oreoList.length" p-2 rounded-md cursor-pointer text="truegray-400 xl"
                    hover="text-truegray-600 bg-truegray-100" @click="generateRandomOreo">
                    <div i-gg-dice-3 />
                </div>
                <div v-show="oreoList.length" p-2 rounded-md cursor-pointer text="truegray-400 xl"
                    hover="text-truegray-600 bg-truegray-100" @click="oreoList = []">
                    <div i-gg-close />
                </div>
            </div>
        ...
        <footer flex items-center justify-center h-16 mx="-8" mb="-8" text="white xl" bg-truegray-700
            :cursor="oreoList.length ? 'pointer' : 'not-allowed'"
            :hover="oreoList.length ? 'bg-truegray-800 text-2xl' : ''" rounded-b-xl select-none @click="handleClick">
            {{ t('input.generate') }}
        </footer>
    </main>
    <SiteFooter />
